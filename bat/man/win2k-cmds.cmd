Window2K 命令大全
accwiz.exe > Accessibility Wizard for walking you through setting up your machine for your

mobility needs. 辅助工具向导

acsetups.exe > ACS setup DCOM server executable

actmovie.exe > Direct Show setup tool 直接显示安装工具

append.exe > Allows programs to open data in specified directories as if they were in the

current directory. 允许程序打开制定目录中的数据

arp.exe > NETWORK Display and modify IP - Hardware addresses 显示和更改计算机的IP与硬件物理地址的对
...
bootvrfy.exe > Bootvrfy.exe, a program included in Windows 2000 that notifies the system that

startup was successful. Bootvrfy.exe can be run on a local or remote computer. 通报启动成功
...
chkdsk.exe > Check the hard disk for errors similar to Scandisk 3 Stages must specify a Drive
...
cipher.exe > Displays or alters the encryption of directories [files] on NTFS partitions. 在
...
clipbrd.exe > Clipboard viewer for Local will allow you to connect to other clipboards 剪贴簿查
...
compact.exe > Displays or alters the compression of files on NTFS partitions. 显示或改变NTFS分区
...
for all COM+ applications. 所有COM+应用软件的主进程
...
lpq.exe > Displays status of a remote LPD queue 显示远端的LPD打印队列的状态，显示被送到基于Unix的服务

器的打印任务

lpr.exe > Send a print job to a network printer. 重定向打印任务到网络中的打印机。通常用于Unix客户打印

机将打印任务发送给连接了打印设备的NT的打印机服务器。
...
nbtstat.exe > Displays protocol stats and current TCP/IP connections using NBT 使用 NBT（TCP/IP
...
nlsfunc.exe > Loads country-specific information 加载特定国家（地区）的信息。Windows 2000 和 MS-DOS

子系统不使用该命令。接受该命令只是为了与 MS-DOS 文件兼容。
...
odbcconf.exe > Configure ODBC driver's and data source's from command line 命令行配置ODBC驱动和数
...
pentnt.exe > Used to check the Pentium for the floating point division error. 检查Pentium的浮点错
...
qprocess.exe > Display information about processes local or remote 在本地或远程显示进程的信息（需终
...
recover.exe > Recovers readable information from a bad or defective disk 从坏的或有缺陷的磁盘中恢
...
register.exe > Register a program so it can have special execution characteristics. 注册包含特殊

运行字符的程序

regsvc.exe >

regsvr32.exe > Registers and unregister's dll's. As to how and where it register's them I dont
...
rexec.exe > Runs commands on remote hosts running the REXEC service. 在运行 REXEC 服务的远程计算机

上运行命令。rexec 命令在执行指定命令前，验证远程计算机上的用户名，只有安装了 TCP/IP 协议后才可以使用该命令
...
router.exe > Router software that runs either on a dedicated DOS or on an OS/2 system. Route软件

在 DOS或者是OS/2系统

rsh.exe > Runs commands on remote hosts running the RSH service 在运行 RSH 服务的远程计算机上运行命
...
rwinsta.exe > Reset the session subsystem hardware and software to known initial values 重置会话
...
schupgr.exe > It will read the schema update files (.ldf files) and upgrade the schema. (part

of ADSI) 读取计划更新文件和更新计划

secedit.exe > Starts Security Editor help 自动安全性配置管理

services.exe > Controls all the services 控制所有服务

sethc.exe > Set High Contrast - changes colours and display mode Logoff to set it back to

normal 设置高对比

setreg.exe > Shows the Software Publishing State Key values 显示软件发布的国家语言

setup.exe > GUI box prompts you to goto control panel to configure system components 安装程序（转
...
tftp.exe > Trivial FTP 将文件传输到正在运行 TFTP 服务的远程计算机或从正在运行 TFTP 服务的远程计算机传输
...
tracert.exe > Trace a route to display paths 该诊断实用程序将包含不同生存时间 (TTL) 值的 Internet 控

制消息协议 (ICMP) 回显数据包发送到目标，以决定到达目标采用的路由
...
utilman.exe > This tool enables an administrator to designate which computers automatically
...
wowdeb.exe > . For starters, the 32-bit APIs require that the WOWDEB.EXE task runs in the

target debugee's VM 启动时，32位API需要
